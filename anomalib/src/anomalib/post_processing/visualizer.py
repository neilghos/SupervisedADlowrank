...
class Visualizer:
    ...
    def visualize_batch(self, batch: dict) -> Iterator[np.ndarray]:
        ...
        for i in range(batch_size):
            ...
            all_img['img'] = image
            all_img['gt'] = batch["mask"][i].squeeze().int().cpu().numpy()
            all_img['heatmap'] = image_result.heat_map
            all_img['pred'] = batch["pred_masks"][i].squeeze().int().cpu().numpy()
            if wandb is not None:
                wandb.log(
                    {
                    "img | gt | heatmap | pred": [wandb.Image(all_img['img']), wandb.Image(all_img['gt']), wandb.Image(all_img['heatmap']), wandb.Image(all_img['pred'])]
                    }
                )
            # (224, 224, 3) (224, 224) (224, 224, 3) (224, 224)
            #print(all_img['img'].shape, all_img['gt'].shape, all_img['heat_map'].shape, all_img['pred'].shape)
            #return all_img
            #exit()
            yield all_img #self.visualize_image(image_result)
