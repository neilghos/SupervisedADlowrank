...
if __name__ == '__main__':
    ...
    if mode == 'train':
        if model in ["low_rank", "padim", "padim_resnet50", "stfpm", "stfpm_resnet50","draem", "cfa", "cflow", "ganomaly", "RD4AD", "patchcore", "patchcore_resnet50"]:
            ...
            print(f'ERROR, you input a wrong model {model}, please select from ["low_rank", "padim", "padim_resnet50", "stfpm", "stfpm_resnet50", "draem", "cfa", "cflow", "ganomaly", "RD4AD", "patchcore", "patchcore_resnet50"]')
    
    elif mode == 'test':
        if model in ["low_rank", "padim", "padim_resnet50", "stfpm", "stfpm_resnet50","draem", "cfa", "cflow", "ganomaly", "RD4AD", "patchcore", "patchcore_resnet50"]:
            config_file = f"config/{data}_{model}.yaml" if os.path.exists(f"config/{data}_{model}.yaml") else f"config/{data}_{model}_test.yaml"
            os.system(f'python anomalib/tools/test.py --model {model} --config {config_file} --weight_file {args.weight}')
        elif model == "UTRAD":
